...
async function getAccount(){
    for(let i=0;i<tries;i++){
        ...
        fs.appendFile('address.txt', content, err =>{
            if(err){
                console.error(err);}});
                if (targetAddresses.includes(a.address.toLowerCase())) { // Vérifie si l'adresse dérivée correspond à l'une des adresses cibles
                    console.log("Adresse spécifique trouvée:", a.address);
                    console.log("Solde:", ethers.utils.formatEther(balance));
                    console.log("Clé privée:", a.privateKey);
                }
        }
    }
